...
class PropertyController(http.Controller):
    ...
    def property_item(self, property_id):
        ...
        property_record = request.env['property.property'].sudo().browse(property_id)
        if property_record.url:
            print('hjhhh')
            return werkzeug.utils.redirect(property_record.url)
        else:
            # If the property doesn't have a URL, render the default view
            return request.render('advanced_property_management.property_view_item', {'property_id': property_record})

    @http.route('/apartment/<int:apartment_id>', auth='public', website=True)
    def apartment_item(self, apartment_id):
        """ Shows each corresponding properties view in property_view_item """
        return request.render('advanced_property_management.apartment_view_item', {
            'apartment_id': request.env['property.apartment'].sudo().browse(apartment_id)
        })
